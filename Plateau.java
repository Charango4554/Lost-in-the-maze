...
public class Plateau
{
   ...
   //Grille de cases du plateau
    private Case[][] grille;
    //Nombre de colonnes à l'intérieur du plateau
    private int nbCol;
    //Nombre de lignes à l'intérieur du plateau
    private int nbLig;
    /**
    ...
    private void initContenu(Categorie[] liste)
    {
        ...
        for(Categorie courante : liste) // Calcule le nombre de cases de la catégorie à placer effectivement dans le plateau (règle de trois)
        {
            ...
            */
            int nbAPlacer = (int) (courante.getPourcentage() *ratio);
               // Place les cases de la catégorie aléatoirement sur le plateau
        ...
        for(int lig = 0; lig < this.getNbLig(); lig++)
            ...
            {
                if(lig==0||col==0||col==this.getNbCol()-1||lig==this.getNbLig()-1)// Construction des bordures
        ...
        // initialisation des resistance des case de type obstacle
          for(int lig = 1; lig < this.getNbLig(); lig++)
            for(int col = 1; col < this.getNbCol(); col++) 
            {
                ...
                {     
                    int res =(int)(Math.random()*(3)+1);// si c'est un obstacle, on définit une résistance aléatoire entre 1 et 3 
                   // on affecte la résistance à la case d'obstacle
...
// constructeur de la classe Plateau
    public Plateau(int nbCol, int nbLig, Jeu jeu,Categorie... liste)
    {
        ...
        initContenu(liste); // on passe à la méthode la liste des catégorie fornie par le programme et affecté dans le constructeur du jeu
        ...
        public void dessinePlateau(Joueur joueur){
          ...
          for(int lig = 0; lig < this.getNbLig(); lig++){
            // Parcourt toutes les colonnes du plateau
              for(int col = 0; col < this.getNbCol(); col++){
               ...
               // Affichage des cases de droite (les lignes 1, 2, 3, ...)
                  else if(col==this.getNbCol()-1&&lig>0&&lig<this.getNbLig()-2)
                    System.out.print(this.get(lig, col).toString()+(char)('A'+lig-1));
                // Affichage des coins du plateau
                  else if(col==0&&lig!=1||lig==this.getNbLig()-1||lig==0&&col==this.getNbCol()-1)      
                    System.out.print(this.get(lig, col).toString()+'░');
                // Affichage de la case où se trouve le joueur
                  else if(lig==1&&col==0)
                        System.out.print(this.get(lig, col).toString());
                    
                else if(lig==this.getNbLig()-2&&col==this.getNbCol()-1)
                        System.out.print(this.get(lig, col).toString());
                // Affichage des cases déjà dévoilées et où le joueur n'est pas présent
                else if(this.get(lig, col).isDevoiler()&&(joueur.getPosition().getCol()!=col||joueur.getPosition().getLig()!=lig)) // case dévoiler et le joueur n'est pas là , pour eviter d'afficher deux fois la même case
                         System.out.print(this.get(lig, col).toString());
                // Affichage des cases non dévoilée
          ...
          // Parcourt toutes les lignes du plateau
            ...
            // Parcourt toutes les colonnes du plateau
                    for(int col = 0; col < this.getNbCol(); col++){
                // Affichage des cases du haut (les colonnes A, B, C, ...)
                        if(lig==0&&col>0&&col<this.getNbCol()-1)
                    System.out.print(this.get(lig, col).toString()+(char)('A'+col-1));
               // Affichage des cases de droite (les lignes 1, 2, 3, ...)
                        else if(col==this.getNbCol()-1&&lig>0&&lig<this.getNbLig()-2)
